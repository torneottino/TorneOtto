...
def export_pdf_tournament(tournament_id):
    ...
    # Partite giocate in totale - conta tutte le partite reali giocate
    total_matches = 0
    for day in completed_days:
        if hasattr(day, 'get_config'):
            config = day.get_config()
            if 'matches' in config:
                # Gironi - conta le partite direttamente
                total_matches += len(config.get('matches', []))
            elif 'schedule' in config:
                # TorneOtto30 - conta tutte le partite dello schedule (3 turni x giornata)
                schedule = config.get('schedule', [])
                for round_matches in schedule:
                    total_matches += len(round_matches)
            elif 'teams' in config:
                # TorneOtto45 - conta dalle squadre
                teams = config.get('teams', [])
                if len(teams) >= 2:
    ...
    pair_matches_count = {}  # Conta partite giocate da ogni coppia
    pair_days_count = {}  # Conta giornate giocate da ogni coppia
    ...
    for day in days_30:
        ...
        # Prima conta le coppie per giornata (non per partita)
        day_pairs = set()
        for team in teams:
            if len(team) >= 2:
                pair = tuple(sorted(team))
                day_pairs.add(pair)
                pair_days_count[pair] = pair_days_count.get(pair, 0) + 1
            ...
            for match in round_matches:
                ...
                if result:
                    try:
                        ...
                        # Conta partite giocate da ogni coppia
                        pair_a = tuple(sorted(team_a_players))
                        pair_b = tuple(sorted(team_b_players))
                        pair_matches_count[pair_a] = pair_matches_count.get(pair_a, 0) + 1
                        pair_matches_count[pair_b] = pair_matches_count.get(pair_b, 0) + 1
    ...
    for day in days_45:
        ...
        # Prima conta le coppie per giornata
        day_pairs = set()
        for match in matches:
            ...
            if len(team_a_players) >= 2:
                ...
                if pair_a not in day_pairs:
                    day_pairs.add(pair_a)
                    pair_days_count[pair_a] = pair_days_count.get(pair_a, 0) + 1
                    
            if len(team_b_players) >= 2:
                pair_b = tuple(sorted(team_b_players[:2]))
                if pair_b not in day_pairs:
                    day_pairs.add(pair_b)
                    pair_days_count[pair_b] = pair_days_count.get(pair_b, 0) + 1
    ...
    for day in days_gironi:
        ...
        matches = day.get_matches()
        results = day.get_results()
        gironi = day.get_gironi() or []
        
         gironi = day.get_gironi() or []
         
         # Conta giorni giocati insieme per ogni coppia  
         for girone in gironi:
             for team in girone:
                 if len(team) >= 2:
                pair = tuple(sorted(team[:2]))
                pair_days_count[pair] = pair_days_count.get(pair, 0) + 1
    ...
    # 1. Coppia più frequente (basato su giornate giocate insieme)
    if pair_days_count:
        most_frequent_pair = max(pair_days_count.items(), key=lambda x: x[1])
        pair_ids = most_frequent_pair[0]
        days_together = most_frequent_pair[1]
        matches_together = pair_matches_count.get(pair_ids, 0)
        
        try:
            pair_players = Player.query.filter(Player.id.in_(pair_ids)).all()
            if len(pair_players) >= 2:
                stats['coppia_frequente'] = f"{pair_players[0].nome} {pair_players[0].cognome} / {pair_players[1].nome} {pair_players[1].cognome} ({days_together} Giornate Assieme / {matches_together} Partite)"
            else:
    ...
    if all_matches_data:
        max_diff_match = max(all_matches_data, key=lambda x: x['diff'])
        try:
            team_a_players = Player.query.filter(Player.id.in_(max_diff_match['team_a'])).all()
            team_b_players = Player.query.filter(Player.id.in_(max_diff_match['team_b'])).all()
            
            team_a_names = [f"{p.cognome}" for p in team_a_players]
            team_b_names = [f"{p.cognome}" for p in team_b_players]
            
            stats['partita_alto_scarto'] = f"{'/'.join(team_a_names)} vs {'/'.join(team_b_names)} ({max_diff_match['score_a']}-{max_diff_match['score_b']}, scarto {max_diff_match['diff']})"
        except:
            stats['partita_alto_scarto'] = f"Partita con scarto {max_diff_match['diff']}"
    else:
        stats['partita_alto_scarto'] = "Nessuna partita trovata"
    
    # 3. Partita con punteggi più bassi
    if all_matches_data:
        min_total_match = min(all_matches_data, key=lambda x: x['total_games'])
        try:
            team_a_players = Player.query.filter(Player.id.in_(min_total_match['team_a'])).all()
            team_b_players = Player.query.filter(Player.id.in_(min_total_match['team_b'])).all()
            
            team_a_names = [f"{p.cognome}" for p in team_a_players]
            team_b_names = [f"{p.cognome}" for p in team_b_players]
            
            stats['partita_bassi_punteggi'] = f"{'/'.join(team_a_names)} vs {'/'.join(team_b_names)} ({min_total_match['score_a']}-{min_total_match['score_b']}, totale {min_total_match['total_games']} games)"
        except:
            stats['partita_bassi_punteggi'] = f"Partita con {min_total_match['total_games']} games totali"
    else:
    ...
    # Restituisce il template HTML stampabile
    return render_template('tournaments/pdf/statistiche_torneo.html',
                         tournament=tournament,
                         stats=stats,
                         now=datetime.now())

# TORNEI AD ELIMINAZIONE - ROUTE PRINCIPALI

@tournaments_bp.route('/tornei/<int:tournament_id>/eliminazione/seleziona-giocatori', methods=['GET', 'POST'])
def elimination_select_players(tournament_id):
    """Selezione giocatori per torneo ad eliminazione"""
    tournament = Tournament.query.get_or_404(tournament_id)
    
    if tournament.tipo_torneo != 'eliminazione':
        flash('Torneo non valido per l\'eliminazione', 'error')
        return redirect(url_for('tournaments.tournaments_list'))
    ...
    if request.method == 'POST':
        ...
        if len(selected_players) != elim_tournament.num_partecipanti:
            flash(f'Devi selezionare esattamente {elim_tournament.num_partecipanti} giocatori', 'error')
            return redirect(url_for('tournaments.elimination_select_players', tournament_id=tournament_id))
        
        # Salva i giocatori selezionati nella configurazione
        config = elim_tournament.get_config()
        config['selected_players'] = [int(p) for p in selected_players]
        elim_tournament.set_config(config)
        
        try:
            db.session.commit()
            
            # Reindirizza alla creazione del tabellone
            return redirect(url_for('tournaments.elimination_create_bracket', tournament_id=tournament_id))
    ...
    players = Player.query.order_by(Player.cognome, Player.nome).all()
    return render_template('tournaments/elimination/player_selection.html', 
                         tournament=tournament, 
                         elim_tournament=elim_tournament,
                         players=players)

@tournaments_bp.route('/tornei/<int:tournament_id>/eliminazione/crea-tabellone', methods=['GET', 'POST'])
def elimination_create_bracket(tournament_id):
    """Creazione del tabellone di eliminazione"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    if not selected_players:
        flash('Nessun giocatore selezionato', 'error')
        return redirect(url_for('tournaments.elimination_select_players', tournament_id=tournament_id))
    
    if request.method == 'POST':
        # Genera il tabellone basato sul metodo selezionato
        try:
            create_elimination_bracket(elim_tournament, selected_players)
            flash('Tabellone creato con successo!', 'success')
            return redirect(url_for('tournaments.elimination_view_bracket', tournament_id=tournament_id))
        except Exception as e:
            flash(f'Errore durante la creazione del tabellone: {str(e)}', 'error')
    
    # GET: mostra l'anteprima del tabellone
    players = Player.query.filter(Player.id.in_(selected_players)).all()
    players_dict = {p.id: p for p in players}
    
    return render_template('tournaments/elimination/bracket_preview.html',
                         tournament=tournament,
                         elim_tournament=elim_tournament,
                         players=players,
                         players_dict=players_dict)

@tournaments_bp.route('/tornei/<int:tournament_id>/eliminazione/tabellone')
def elimination_view_bracket(tournament_id):
    """Visualizza il tabellone di eliminazione"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    matches = EliminationMatch.query.filter_by(tournament_id=elim_tournament.id).order_by(
        EliminationMatch.turno, EliminationMatch.posizione_turno).all()
    ...
    for team in teams:
        if not team.is_bye:
            all_player_ids.extend(team.get_players())
    ...
    return render_template('tournaments/elimination/bracket_view.html',
                         tournament=tournament,
                         elim_tournament=elim_tournament,
                         matches=matches,
...
@tournaments_bp.route('/tornei/<int:tournament_id>/eliminazione/inserisci-risultati', methods=['GET', 'POST'])
def elimination_enter_results(tournament_id):
    ...
    if request.method == 'POST':
        ...
        try:
            process_elimination_results(elim_tournament, request.form)
            flash('Risultati salvati con successo!', 'success')
            return redirect(url_for('tournaments.elimination_view_bracket', tournament_id=tournament_id))
        except Exception as e:
            flash(f'Errore durante il salvataggio: {str(e)}', 'error')
    
    # GET: mostra le partite da completare
    pending_matches = EliminationMatch.query.filter_by(
        tournament_id=elim_tournament.id, 
        stato='Pending'
    ).filter(
        EliminationMatch.team1_id.isnot(None), 
        EliminationMatch.team2_id.isnot(None)
    ).order_by(EliminationMatch.turno, EliminationMatch.posizione_turno).all()
    
    # Recupera i giocatori per le squadre
    all_player_ids = []
    for match in pending_matches:
        if match.team1 and not match.team1.is_bye:
            all_player_ids.extend(match.team1.get_players())
        if match.team2 and not match.team2.is_bye:
            all_player_ids.extend(match.team2.get_players())
    
    players = Player.query.filter(Player.id.in_(all_player_ids)).all()
    players_dict = {p.id: p for p in players}
    
    return render_template('tournaments/elimination/enter_results.html',
                         tournament=tournament,
                         elim_tournament=elim_tournament,
...
def create_elimination_bracket(elim_tournament, selected_players):
    """Crea il tabellone di eliminazione"""
    ...
    # Raggruppa i giocatori a coppie
    players_copy = selected_players.copy()
    
    if elim_tournament.metodo_accoppiamento == 'random':
        random.shuffle(players_copy)
    elif elim_tournament.metodo_accoppiamento == 'elo':
        # Ordina per ELO (implementa la logica ELO se necessario)
        pass
    
    # Crea le squadre da 2 giocatori
    for i in range(0, len(players_copy), 2):
        team_players = players_copy[i:i+2]
        if len(team_players) == 1:
            # Numero dispari di giocatori, aggiungi un "bye"
            team_players.append(None)
        
        team = EliminationTeam(
            tournament_id=elim_tournament.id,
            posizione_seed=len(teams) + 1
        )
        team.set_players([p for p in team_players if p is not None])
        teams.append(team)
        db.session.add(team)
    
    # Aggiungi squadre "bye" se necessario per raggiungere la potenza di 2
    elim_tournament.num_squadre = elim_tournament.get_next_power_of_2(len(teams))
    
    while len(teams) < elim_tournament.num_squadre:
    ...
    db.session.flush()  # Per ottenere gli ID delle squadre
    
    # Crea il primo turno di partite
    ...
    if elim_tournament.tipo_eliminazione == 'double':
        create_double_elimination_bracket(elim_tournament, teams)
    
    # Aggiorna lo stato del torneo
    elim_tournament.stato = 'InProgress'
    db.session.commit()
...
def create_next_round_matches(elim_tournament, completed_match):
    ...
    from models.elimin_day import EliminationMatch
    
    # Logica per creare la prossima partita
