...
def new_americana_day(tournament_id):
    """Crea una nuova giornata per un torneo all'americana"""
    # Recupera il torneo
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    return render_template('tournaments/americana/day_setup.html', 
                          tournament_id=tournament_id,
                          tournament=tournament,
                          players=players,
                          data_oggi=data_oggi)

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/risultati-americana', methods=['GET'])
def view_americana_day(tournament_id, day_id):
    """Visualizza la pagina per l'inserimento dei risultati di una giornata all'americana"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    # Debug logging
    app.logger.info(f"View americana day {day_id}: stato={day.stato}")
    ...
    return render_template('tournaments/americana/day_results.html',
                          tournament=tournament,
                          day=day,
                          matches=matches,
                          results=results,
...
@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/americana/players', methods=['POST'])
def americana_players(tournament_id):
    """Gestisce il form di setup della giornata americana e passa alla selezione giocatori"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    # Validazione
    if not all([data, num_coppie, metodo_coppie]):
        flash('Tutti i campi sono obbligatori!', 'error')
        return redirect(url_for('tournaments.new_americana_day', tournament_id=tournament_id))
    
    if num_coppie < 4 or num_coppie > 16:
        flash('Il numero di coppie deve essere tra 4 e 16!', 'error')
        return redirect(url_for('tournaments.new_americana_day', tournament_id=tournament_id))
    
    if metodo_coppie not in ['casuale', 'elo', 'seeded', 'manuale', 'coppie_fisse']:
