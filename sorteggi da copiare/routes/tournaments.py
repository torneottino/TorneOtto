...
def create_americana_tournament():
    ...
    if request.method == 'POST':
        ...
        # Mappatura valori form -> backend
        metodo_punteggio_map = {
            'differenza_games': 'game_difference',
            'punti_set': 'set_points',
            'punti_partita': 'match_points'
        }
        metodo_punteggio = metodo_punteggio_map.get(metodo_punteggio, 'match_points')
...
def calculate_torneotto45_elo(tournament_id, day_id):
    ...
    """
    from models.player import PlayerEloHistory, PlayerTournamentElo
    ...
    if not classifica or not semifinali or not finali:
        app.logger.error(f"Classifica, semifinali o finali mancanti per giornata {day_id}")
        return
    ...
    final_changes = {}
    for player_id in classifica:
        total_change = sum(player_changes[player_id])
        final_changes[player_id] = total_change
        app.logger.info(f"Giocatore {player_id}: Variazione ELO totale = {total_change}")
    
    # 4. Applica le variazioni ELO e salva lo storico
    for player_id, elo_change in final_changes.items():
        # Ottieni o crea l'ELO del giocatore per questo torneo
        player_elo = PlayerTournamentElo.query.filter_by(
            player_id=player_id,
            tournament_id=tournament_id
        ).first()
        ...
        if not player_elo:
            # Crea nuovo record ELO se non esiste
            player_elo = PlayerTournamentElo(
                player_id=player_id,
                tournament_id=tournament_id,
                elo_rating=1500  # ELO di partenza
            )
            db.session.add(player_elo)
        
        # Salva lo storico prima di modificare l'ELO
        elo_history = PlayerEloHistory(
            player_id=player_id,
            tournament_id=tournament_id,
            tournament_day_id=day_id,
            old_elo=player_elo.elo_rating,
            new_elo=player_elo.elo_rating + elo_change,
            elo_change=elo_change
        )
        db.session.add(elo_history)
        
        # Aggiorna l'ELO del giocatore
        player_elo.elo_rating += elo_change
        
        app.logger.info(f"Giocatore {player_id}: ELO aggiornato da {elo_history.old_elo} a {elo_history.new_elo}")
    
    # Commit delle modifiche
    db.session.commit()
    app.logger.info("Calcolo ELO completato con successo")

def calculate_match_elo_change(team1_elo, team2_elo, match_result, k_factor=32):
    """
    Calcola la variazione ELO per una partita specifica
    
    Args:
        team1_elo: ELO totale della squadra 1
        team2_elo: ELO totale della squadra 2
        match_result: Risultato per la squadra 1 (1=vittoria, 0.5=pareggio, 0=sconfitta)
        k_factor: Fattore K per il calcolo ELO
    ...
    team1_change = k_factor * (match_result - expected_score_team1)
    team2_change = k_factor * ((1 - match_result) - expected_score_team2)
    
    return team1_change, team2_change

def get_player_current_elo(player_id, tournament_id, before_day_id=None):
    """
    Ottieni l'ELO attuale di un giocatore per un torneo specifico
    
    Args:
        player_id: ID del giocatore
        tournament_id: ID del torneo
        before_day_id: Se specificato, ottieni l'ELO prima di questa giornata
    
    Returns:
        int: ELO attuale del giocatore
    """
    from models.player import PlayerTournamentElo
    ...
    if player_elo:
        return player_elo.elo_rating
    else:
        return 1500  # ELO di default per nuovi giocatori
...
def torneotto45_players(tournament_id):
    """Selezione giocatori per TorneOtto 45"""
    tournament = Tournament.query.get_or_404(tournament_id)
    
    selected_players = request.form.getlist('selected_players')
    
    if len(selected_players) != 8:
        flash('Devi selezionare esattamente 8 giocatori per il TorneOtto 45!', 'error')
        return redirect(url_for('tournaments.new_torneotto45_day', tournament_id=tournament_id))
    
    # Salva la selezione nella sessione
    session['torneotto45_players'] = selected_players
    session['torneotto45_tournament_id'] = tournament_id
    
    return redirect(url_for('tournaments.torneotto45_choose_pairing_method', tournament_id=tournament_id))
...
def torneotto45_choose_pairing_method(tournament_id):
    """Scelta del metodo di abbinamento per TorneOtto 45"""
    tournament = Tournament.query.get_or_404(tournament_id)
    
    # Verifica che ci siano giocatori selezionati
    if 'torneotto45_players' not in session:
        flash('Devi prima selezionare i giocatori!', 'error')
        return redirect(url_for('tournaments.new_torneotto45_day', tournament_id=tournament_id))
    
    selected_players = session['torneotto45_players']
...
def torneotto45_random_pairing(tournament_id):
    """Abbinamento casuale per TorneOtto 45"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    selected_players = session['torneotto45_players']
    random.shuffle(selected_players)  # Mescola casualmente
    
    # Crea le semifinali
    semifinali = [
        {
            'squadra_a': [int(selected_players[0]), int(selected_players[1])],
            'squadra_b': [int(selected_players[2]), int(selected_players[3])],
            'risultato': None
        },
        {
            'squadra_a': [int(selected_players[4]), int(selected_players[5])],
            'squadra_b': [int(selected_players[6]), int(selected_players[7])],
            'risultato': None
        }
    ]
    
    # Salva nella sessione
    session['torneotto45_semifinals'] = semifinali
    
    return redirect(url_for('tournaments.save_torneotto45_day', tournament_id=tournament_id))
...
def process_torneotto45_random_pairing(tournament_id):
    """Processa l'abbinamento casuale per TorneOtto 45"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    shuffled_players = selected_players.copy()
    random.shuffle(shuffled_players)
    
    # Crea le semifinali con i giocatori mescolati
    semifinali = [
        {
            'squadra_a': [int(shuffled_players[0]), int(shuffled_players[1])],
            'squadra_b': [int(shuffled_players[2]), int(shuffled_players[3])],
            'risultato': None
        },
        {
            'squadra_a': [int(shuffled_players[4]), int(shuffled_players[5])],
            'squadra_b': [int(shuffled_players[6]), int(shuffled_players[7])],
            'risultato': None
        }
    ]
    
    # Salva le semifinali nella sessione
    session['torneotto45_semifinals'] = semifinali
...
def torneotto45_elo_pairing(tournament_id):
    """Abbinamento basato su ELO per TorneOtto 45"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    # Ottieni gli ELO dei giocatori
    player_elos = []
    for player_id in selected_players:
        elo = get_player_current_elo(int(player_id), tournament_id)
        player_elos.append((int(player_id), elo))
    
    # Ordina per ELO decrescente
    player_elos.sort(key=lambda x: x[1], reverse=True)
    
    # Abbina i giocatori bilanciando gli ELO
    # Giocatori con ELO simile vengono messi in squadre diverse
    semifinali = [
        {
            'squadra_a': [player_elos[0][0], player_elos[3][0]],  # 1° + 4°
            'squadra_b': [player_elos[1][0], player_elos[2][0]],  # 2° + 3°
            'risultato': None
        },
        {
            'squadra_a': [player_elos[4][0], player_elos[7][0]],  # 5° + 8°
            'squadra_b': [player_elos[5][0], player_elos[6][0]],  # 6° + 7°
            'risultato': None
        }
    ]
...
def new_americana_day(tournament_id):
    """Crea una nuova giornata americana"""
    tournament = Tournament.query.get_or_404(tournament_id)
    
    if tournament.tipo_torneo != 'americana':
        flash('Questo torneo non è di tipo americana!', 'error')
        return redirect(url_for('tournaments.view_tournament', tournament_id=tournament_id))
    
    # Ottieni la configurazione del torneo
    config = tournament.get_config()
    # Imposta valori di default se non ci sono
    if not config.get('num_coppie'):
        config['num_coppie'] = 8
    if not config.get('num_campi'):
        config['num_campi'] = 4
    if not config.get('tipo_torneo'):
        config['tipo_torneo'] = 'completo'
    if not config.get('origine_giocatori'):
        config['origine_giocatori'] = 'caricati'
    
    return render_template('tournaments/americana/player_selection.html', tournament=tournament, config=config)

@tournaments_bp.route('/tornei/<int:tournament_id>/americana/players', methods=['POST'])
def americana_players(tournament_id):
    """Gestione selezione giocatori per torneo americana"""
    tournament = Tournament.query.get_or_404(tournament_id)
    
    # Ottieni la configurazione del torneo
    config = tournament.get_config()
    
    # Determina l'origine dei giocatori
    config['origine_giocatori'] = request.form.get('origine_giocatori', 'caricati')
    
    # Salva la configurazione aggiornata
    tournament.set_config(config)
    db.session.commit()
    
    if config['origine_giocatori'] == 'caricati':
        # Usa giocatori già caricati nel sistema
...
def americana_setup(tournament_id):
    ...
    player_names = [f"{p['nome']} {p['cognome']}" for p in players]
    ...
    try:
        ...
        # Genera le partite usando l'algoritmo corretto in base al tipo di torneo
        tournament_type = config.get('tipo_torneo', 'semplice')
        
        if config.get('tipo_coppie') == 'fisse':
            # Per coppie fisse, usa l'algoritmo di round-robin per squadre
            matches = AmericanaService.generate_round_robin_matches(teams)
        else:  # coppie giranti
            if tournament_type == 'semplice':
                # Per torneo semplice con coppie giranti: usa il nuovo algoritmo con turni/campi
                num_rounds_config = int(request.form.get('num_turni_semplice', 5))
                num_courts = config.get('num_campi', 4)
                rounds = AmericanaService.generate_tournament_schedule(players, num_courts, num_rounds_config)
                # Flatten dei match da tutti i round
                matches = [m for round_matches in rounds for m in round_matches]
            else:  # completo
                # Per torneo completo: round-robin completo
        ...
        return redirect(url_for('tournaments.americana_draw_animation', 
                               tournament_id=tournament_id, 
                               day_id=day.id))
...
def view_americana_day(tournament_id, day_id):
    ...
    if results:
        if config.get('tipo_coppie') == 'fisse':
            ...
            ranking = AmericanaService.calculate_player_ranking(
                matches, results, config.get('metodo_punteggio', 'match_points')
            )
    ...
    if request.method == 'POST':
        try:
            ...
            for key, value in request.form.items():
                if key.startswith('match_') and key.endswith('_team1_games'):
                    match_id = key.replace('match_', '').replace('_team1_games', '')
                    ...
                    if team1_games or team2_games:  # Solo se sono stati inseriti dei risultati
                        # Calcola il vincitore in automatico
            ...
            # Reindirizza alla visualizzazione del torneo
...
def americana_ranking(tournament_id, day_id):
    ...
    # Calcola la classifica e statistiche
    ranking = []
    statistics = {}
    ...
    if results:
        try:
            ...
            flash(f'Errore nel calcolo della classifica: {str(e)}', 'error')
            ranking = []
            statistics = {}
...
def americana_statistics(tournament_id, day_id):
    """Visualizza le statistiche del torneo americana"""
    from models.tournament_day import AmericanaDay
    from services.americana_service import AmericanaService
    ...
    if results:
        try:
            ...
            flash(f'Errore nel calcolo delle statistiche: {str(e)}', 'error')
            statistics = {}
...
@tournaments_bp.route('/tornei/<int:tournament_id>/americana/<int:day_id>/draw-animation')
def americana_draw_animation(tournament_id, day_id):
    """Animazione di sorteggio per americana, poi redirect automatico alla giornata."""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = AmericanaDay.query.get_or_404(day_id)
    return render_template('tournaments/americana/draw_animation.html',
                          tournament=tournament,
                          day=day)

@tournaments_bp.route('/tornei/<int:tournament_id>/americana/<int:day_id>/conferma', methods=['POST'])
def confirm_americana_day(tournament_id, day_id):
    from models.tournament_day import AmericanaDay
    day = AmericanaDay.query.get_or_404(day_id)
    if day.tournament_id != tournament_id:
        flash('Giornata non valida per questo torneo!', 'error')
    ...
    day.stato = 'Aperta'
    db.session.commit()
    flash('Giornata confermata e salvata!','success')
    return redirect(url_for('tournaments.view_americana_day', tournament_id=tournament_id, day_id=day_id))
