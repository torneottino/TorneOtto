...
class Tournament(db.Model):
    ...
    # Relazione con i tornei ad eliminazione
    elimination_tournament = db.relationship('EliminationTournament', backref='base_tournament', lazy=True, cascade='all, delete-orphan', uselist=False)
    ...
    def get_eliminazione_config(self):
        ...
        config = self.get_config()
        return {
            'tipo_eliminazione': config.get('tipo_eliminazione', 'single'),
            'num_partecipanti': config.get('num_partecipanti', 16),
            'metodo_accoppiamento': config.get('metodo_accoppiamento', 'random'),
            'best_of_three': config.get('best_of_three', False)
        } 