...
from models.tournament_day import TorneOtto30Day, TorneOtto45Day, TournamentDay, GironiDay
...
def new_gironi_day(tournament_id):
    """Creazione di una nuova giornata per un torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    if request.method == 'POST':
        ...
        metodo_sorteggio = request.form.get('metodo_sorteggio')
        
        if not all([data, num_coppie, num_gironi, metodo_sorteggio]):
            flash('Tutti i campi sono obbligatori!', 'error')
            return render_template('tournaments/gironi/new_day.html', tournament=tournament)
        
        # Verifica che il numero di coppie sia compatibile con il numero di gironi
        if num_coppie < num_gironi * 2:
            flash('Il numero di coppie deve essere almeno il doppio del numero di gironi!', 'error')
            return render_template('tournaments/gironi/new_day.html', tournament=tournament)
        
        # Reindirizza alla selezione dei giocatori
        return redirect(url_for('tournaments.select_gironi_players', 
                              tournament_id=tournament_id,
                              data=data,
                              num_coppie=num_coppie,
                              num_gironi=num_gironi,
                              metodo_sorteggio=metodo_sorteggio))
    
    return render_template('tournaments/gironi/new_day.html', tournament=tournament)

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/gironi/players', methods=['GET', 'POST'])
def select_gironi_players(tournament_id):
    """Selezione dei giocatori per una giornata del torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    if request.method == 'POST':
        ...
        if len(selected_players) != num_coppie * 2:
            flash(f'Devi selezionare esattamente {num_coppie * 2} giocatori!', 'error')
            return redirect(url_for('tournaments.select_gironi_players', 
                                  tournament_id=tournament_id,
                                  data=data,
                                  num_coppie=num_coppie,
        ...
        # Se il metodo è "seeded", reindirizza alla pagina di selezione delle teste di serie
        if metodo_sorteggio == "seeded":
        ...
        # Altrimenti procedi con l'animazione del sorteggio
        return redirect(url_for('tournaments.gironi_pairing_animation_new',
                              tournament_id=tournament_id,
                              data=data,
                              num_coppie=num_coppie,
                              num_gironi=num_gironi,
        ...
        metodo_sorteggio = request.args.get('metodo_sorteggio')
    ...
    return render_template('tournaments/gironi/select_players.html',
                         tournament=tournament,
                         players=players,
                         num_players=num_coppie * 2,
                         num_coppie=num_coppie,
                         num_gironi=num_gironi,
                         data=data,
                         metodo_sorteggio=metodo_sorteggio)

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/gironi/pairing', methods=['GET', 'POST'])
def gironi_pairing_animation_new(tournament_id):
    """Animazione del sorteggio delle coppie e dei gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    data = request.args.get('data')
    num_coppie = int(request.args.get('num_coppie'))
    num_gironi = int(request.args.get('num_gironi'))
    ...
    # Recupera i giocatori e convertili in dizionari
    players = []
    ...
    return render_template('tournaments/gironi/pairing_animation.html',
                         tournament=tournament,
                         players=players,
                         date=data,
                         num_coppie=num_coppie,
                         num_gironi=num_gironi,
                         method=metodo_sorteggio)

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/gironi/save', methods=['POST'])
def save_gironi_day(tournament_id):
    """Salva i dati base di una nuova giornata del torneo a gironi e reindirizza alla selezione giocatori"""
    try:
        tournament = Tournament.query.get_or_404(tournament_id)
        ...
        if not num_coppie:
            raise ValueError("Il numero di coppie è obbligatorio")
            
        num_gironi = request.form.get('num_gironi')
        if not num_gironi:
            raise ValueError("Il numero di gironi è obbligatorio")
            
        metodo_sorteggio = request.form.get('metodo_sorteggio')
        if not metodo_sorteggio:
            raise ValueError("Il metodo di sorteggio è obbligatorio")
        
        # Crea la giornata con i dati base
        day = GironiDay(
            tournament_id=tournament_id,
        ...
        # Reindirizza alla pagina di selezione giocatori
        return redirect(url_for('tournaments.select_gironi_players', 
                              tournament_id=tournament_id,
                              data=data,
                              num_coppie=num_coppie,
...
def enter_gironi_results(tournament_id, day_id):
    """Pagina per l'inserimento dei risultati di una giornata del torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    return render_template('tournaments/gironi/enter_results.html',
                         tournament=tournament,
                         day=day,
                         matches=day.get_config()['matches'],
                         num_gironi=len(day.get_config()['groups']))

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/process-gironi-results', methods=['POST'])
def process_gironi_results(tournament_id, day_id):
    """Processa i risultati dei gironi e genera le semifinali"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    
    try:
        # Aggiorna i risultati dei gironi
        config = day.get_config()
        for match in config['matches']:
            match_id = match['id']
            match['risultato_team1'] = int(request.form.get(f'result_{match_id}_team1'))
            match['risultato_team2'] = int(request.form.get(f'result_{match_id}_team2'))
        
        # Calcola le classifiche dei gironi e genera le semifinali
        rankings = calculate_group_rankings(config['matches'], config['groups'])
        semifinals = generate_semifinals(rankings)
        ...
        # Restituisci l'HTML delle semifinali
        semifinals_html = render_template('tournaments/gironi/_semifinals.html',
                                       semifinals=semifinals)
        
        return jsonify({
            'success': True,
            'semifinals_html': semifinals_html
        })
...
@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/process-semifinal-results', methods=['POST'])
def process_semifinal_results(tournament_id, day_id):
    """Processa i risultati delle semifinali e genera le finali"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    ...
    try:
        # Aggiorna i risultati delle semifinali
        data = day.get_data()
        for match in data['semifinals']:
            match_id = match['id']
            match['risultato_team1'] = int(request.form.get(f'result_{match_id}_team1'))
            match['risultato_team2'] = int(request.form.get(f'result_{match_id}_team2'))
        
        # Genera le finali
        finals = generate_finals(data['semifinals'])
        ...
        # Restituisci l'HTML delle finali
        finals_html = render_template('tournaments/gironi/_finals.html',
                                    finals=finals)
        
        return jsonify({
            'success': True,
            'finals_html': finals_html
        })
...
@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/chiudi-gironi', methods=['POST'])
def close_gironi_day(tournament_id, day_id):
    """Chiude una giornata del torneo a gironi e aggiorna gli ELO"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    ...
    try:
        # Aggiorna i risultati delle finali
        data = day.get_data()
        for match in data['finals']:
            match_id = match['id']
            match['risultato_team1'] = int(request.form.get(f'result_{match_id}_team1'))
            match['risultato_team2'] = int(request.form.get(f'result_{match_id}_team2'))
        
        # Calcola la classifica finale
        final_ranking = calculate_final_ranking(data['finals'])
        
        # Aggiorna gli ELO
        update_gironi_elos(tournament_id, day_id, data)
        
        # Aggiorna i dati della giornata
        data['final_ranking'] = final_ranking
        day.set_data(data)
        day.stato = 'Completata'
        
        db.session.commit()
        
        flash('Giornata completata con successo!', 'success')
        return redirect(url_for('tournaments.view_tournament_day', day_id=day_id))
...
@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/export-gironi-pdf')
def export_gironi_pdf(tournament_id, day_id):
    """Esporta il riepilogo della giornata in formato PDF"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    ...
    html = render_template('tournaments/gironi/export_pdf.html',
                         tournament=tournament,
                         day=day,
                         teams=data['teams'],
                         ...
                         num_gironi=len(data['groups']))
    
    # Genera il PDF
    pdf = generate_pdf(html)
    
    # Crea il nome del file
    filename = f"torneo_{tournament.id}_giornata_{day.id}.pdf"
...
def calculate_group_rankings(matches, groups):
    """Calcola le classifiche dei gironi"""
    rankings = {}
    
    for group_num, group in enumerate(groups, 1):
        group_matches = [m for m in matches if m['girone'] == group_num]
        group_teams = {}
        ...
        for match in group_matches:
            if match['risultato_team1'] is not None and match['risultato_team2'] is not None:
                team1_id = match['team1']['id']
                team2_id = match['team2']['id']
                
                diff = match['risultato_team1'] - match['risultato_team2']
                
                if diff > 0:  # Vittoria team1
                    group_teams[team1_id]['punti'] += 3
                    group_teams[team1_id]['vittorie'] += 1
                    group_teams[team2_id]['sconfitte'] += 1
                elif diff < 0:  # Vittoria team2
                    group_teams[team2_id]['punti'] += 3
                    group_teams[team2_id]['vittorie'] += 1
                    group_teams[team1_id]['sconfitte'] += 1
                else:  # Pareggio
                    group_teams[team1_id]['punti'] += 1
                    group_teams[team2_id]['punti'] += 1
                    group_teams[team1_id]['pareggi'] += 1
                    group_teams[team2_id]['pareggi'] += 1
                
                group_teams[team1_id]['differenza'] += diff
                group_teams[team2_id]['differenza'] -= diff
        
        # Ordina le squadre
        rankings[group_num] = sorted(
            group_teams.values(),
            key=lambda x: (-x['punti'], -x['differenza'], -x['vittorie'])
        )
    
    return rankings

def generate_semifinals(rankings):
    """Genera le semifinali basate sulle classifiche dei gironi"""
    semifinals = []
    
    # Prendi i primi due di ogni girone
    qualified_teams = []
    for group_num, ranking in rankings.items():
        qualified_teams.extend(ranking[:2])
    
    # Ordina per punti totali
    qualified_teams.sort(key=lambda x: (-x['punti'], -x['differenza'], -x['vittorie']))
    
    # Genera le semifinali (1° vs 4°, 2° vs 3°)
    semifinals = [
        {
            'id': 'semi1',
            'team1': qualified_teams[0]['team'],
            'team2': qualified_teams[3]['team'],
            'risultato_team1': None,
            'risultato_team2': None
        },
        {
            'id': 'semi2',
            'team1': qualified_teams[1]['team'],
            'team2': qualified_teams[2]['team'],
            'risultato_team1': None,
            'risultato_team2': None
        }
    ]
...
def generate_finals(semifinals):
    """Genera le finali basate sui risultati delle semifinali"""
    finals = []
    
    # Determina i vincitori e perdenti delle semifinali
    winners = []
    losers = []
    for semi in semifinals:
        if semi['risultato_team1'] > semi['risultato_team2']:
            winners.append(semi['team1'])
            losers.append(semi['team2'])
        else:
            winners.append(semi['team2'])
            losers.append(semi['team1'])
...
def calculate_final_ranking(finals):
    """Calcola la classifica finale basata sui risultati delle finali"""
    final_ranking = []
    
    # Finale 1°-2° posto
    if finals[0]['risultato_team1'] > finals[0]['risultato_team2']:
        final_ranking.extend([finals[0]['team1'], finals[0]['team2']])
    else:
        final_ranking.extend([finals[0]['team2'], finals[0]['team1']])
    
    # Finale 3°-4° posto
    if finals[1]['risultato_team1'] > finals[1]['risultato_team2']:
        final_ranking.extend([finals[1]['team1'], finals[1]['team2']])
    else:
        final_ranking.extend([finals[1]['team2'], finals[1]['team1']])
    
    return final_ranking

def update_gironi_elos(tournament_id, day_id, data):
    """Aggiorna gli ELO dei giocatori basati sui risultati della giornata"""
    # Aggiorna gli ELO per le partite dei gironi
    for match in data['matches']:
        if match['risultato_team1'] is not None and match['risultato_team2'] is not None:
            team1_elo = get_team_elo(match['team1'], tournament_id)
            team2_elo = get_team_elo(match['team2'], tournament_id)
            
            result = 1 if match['risultato_team1'] > match['risultato_team2'] else (
                0.5 if match['risultato_team1'] == match['risultato_team2'] else 0
            )
            
            elo_change = calculate_match_elo_change(team1_elo, team2_elo, result)
            
            # Aggiorna gli ELO dei giocatori
            for player in [match['team1']['player1'], match['team1']['player2']]:
                update_player_elo(player['id'], tournament_id, elo_change)
            
            for player in [match['team2']['player1'], match['team2']['player2']]:
                update_player_elo(player['id'], tournament_id, -elo_change)
    
    # Aggiorna gli ELO per le semifinali
    for match in data['semifinals']:
        team1_elo = get_team_elo(match['team1'], tournament_id)
        team2_elo = get_team_elo(match['team2'], tournament_id)
        ...
        elo_change = calculate_match_elo_change(team1_elo, team2_elo, result, k_factor=40)
    ...
    # Aggiorna gli ELO per le finali
    for match in data['finals']:
        team1_elo = get_team_elo(match['team1'], tournament_id)
        team2_elo = get_team_elo(match['team2'], tournament_id)
        ...
        # K-factor più alto per le finali
        k_factor = 50 if match['id'] == 'final1' else 40
        elo_change = calculate_match_elo_change(team1_elo, team2_elo, result, k_factor=k_factor)
...
def update_player_elo(player_id, tournament_id, elo_change):
    """Aggiorna l'ELO di un giocatore per un torneo specifico"""
    player_tournament_elo = PlayerTournamentElo.query.filter_by(
        player_id=player_id,
        tournament_id=tournament_id
    ).first()
    
    if player_tournament_elo:
        old_elo = player_tournament_elo.elo
        new_elo = old_elo + elo_change
        
        # Aggiorna l'ELO del torneo
        player_tournament_elo.elo = new_elo
        
        # Registra la variazione nella storia
        elo_history = PlayerEloHistory(
            player_id=player_id,
            tournament_id=tournament_id,
            old_elo=old_elo,
            new_elo=new_elo,
            variation=elo_change
        )
        
        db.session.add(elo_history)
    else:
        # Se non esiste un ELO per il torneo, crealo
        player = Player.query.get(player_id)
        new_elo = player.elo_standard + elo_change
        
        player_tournament_elo = PlayerTournamentElo(
            player_id=player_id,
            tournament_id=tournament_id,
            elo=new_elo
        )
        ...
        elo_history = PlayerEloHistory(
            player_id=player_id,
            tournament_id=tournament_id,
            old_elo=player.elo_standard,
            new_elo=new_elo,
            variation=elo_change
        )
        
        db.session.add(player_tournament_elo)
        db.session.add(elo_history)

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/gironi')
def view_gironi_day(tournament_id, day_id):
    """Visualizza una giornata del torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    ...
    # Recupera i dati della giornata
    config = day.get_config()
    teams = config.get('teams', [])
    groups = config.get('groups', [])
    matches = config.get('matches', [])
    rankings = config.get('rankings', {})
    semifinals = config.get('semifinals', [])
    finals = config.get('finals', [])
    final_ranking = config.get('final_ranking', [])
    
    # Recupera i dettagli dei giocatori
    player_ids = set()
    for team in teams:
        if isinstance(team, dict):
            # Se il team è un dizionario, estrai gli ID dalla chiave 'players'
            if 'players' in team and isinstance(team['players'], list):
                player_ids.update(pid for pid in team['players'] if isinstance(pid, (int, str)) and str(pid).isdigit())
        elif isinstance(team, list):
            # Se il team è una lista, usa direttamente gli elementi come ID
            player_ids.update(pid for pid in team if isinstance(pid, (int, str)) and str(pid).isdigit())
    
    # Converti gli ID in interi
    player_ids = [int(pid) for pid in player_ids]
    
    if not player_ids:
        flash('Nessun giocatore trovato nelle squadre', 'error')
        return redirect(url_for('tournaments.view_tournament', tournament_id=tournament_id))
    ...
    for team in teams:
        ...
        if isinstance(team, dict) and 'players' in team:
            # Se il team è un dizionario, usa la chiave 'players'
            team_players = [player_dict.get(int(pid)) for pid in team['players'] 
                          if isinstance(pid, (int, str)) and str(pid).isdigit() and int(pid) in player_dict]
        elif isinstance(team, list):
            # Se il team è una lista, usa direttamente gli elementi
            team_players = [player_dict.get(int(pid)) for pid in team 
                          if isinstance(pid, (int, str)) and str(pid).isdigit() and int(pid) in player_dict]
        
        if len(team_players) == 2:  # Verifica che ci siano esattamente 2 giocatori
            teams_objects.append(team_players)
    
    return render_template('tournaments/gironi/view_day.html',
                         tournament=tournament,
                         day=day,
                         teams=teams_objects,
                         groups=groups,
                         ...
                         num_gironi=len(groups))

@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/gironi/select-players', methods=['GET', 'POST'])
def select_gironi_players_for_day(tournament_id, day_id):
    """Pagina per la selezione dei giocatori di una giornata del torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    ...
    if request.method == 'POST':
        ...
        if len(players) < 4:
            flash('Seleziona almeno 4 giocatori', 'error')
            return redirect(url_for('tournaments.select_gironi_players_for_day', tournament_id=tournament_id, day_id=day_id))
        
        # Salva i giocatori selezionati
        day.set_data({'selected_players': players})
        db.session.commit()
        
        return redirect(url_for('tournaments.gironi_pairing_animation', 
                              tournament_id=tournament_id, 
                              day_id=day_id,
                              data=day.data.strftime('%Y-%m-%d'),
                              players=','.join(players)))
    
    # GET: mostra la pagina di selezione
    players = Player.query.order_by(Player.cognome, Player.nome).all()
    selected_players = day.get_config().get('selected_players', [])
...
@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/<int:day_id>/gironi/pairing', methods=['GET', 'POST'])
def gironi_pairing_animation(tournament_id, day_id):
    """Pagina per la formazione delle coppie di una giornata del torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    day = GironiDay.query.get_or_404(day_id)
    ...
    if request.method == 'POST':
        try:
            # Recupera i dati delle coppie formate
            teams_data = json.loads(request.form.get('teams'))
            groups_data = json.loads(request.form.get('groups'))
            
            # Aggiorna i dati della giornata
            config = day.get_config()
            config['teams'] = teams_data
            config['groups'] = groups_data
            day.set_config(config)
            
            db.session.commit()
            
            flash('Coppie formate con successo!', 'success')
            return redirect(url_for('tournaments.view_gironi_day', tournament_id=tournament_id, day_id=day_id))
            
        except Exception as e:
            flash(f'Errore durante la formazione delle coppie: {str(e)}', 'error')
            return redirect(url_for('tournaments.gironi_pairing_animation', tournament_id=tournament_id, day_id=day_id))
    
    # Recupera i dati della giornata
    config = day.get_config()
    num_coppie = config['num_coppie']
    num_gironi = config['num_gironi']
    metodo_sorteggio = config['metodo_sorteggio']
    
    # Recupera i giocatori selezionati
    selected_players = request.form.getlist('players')
    players = Player.query.filter(Player.id.in_(selected_players)).all()
...
@tournaments_bp.route('/tornei/<int:tournament_id>/giornate/gironi/save-from-pairing', methods=['POST'])
def save_gironi_day_from_pairing(tournament_id):
    """Salva i dati delle coppie e dei gironi di una giornata del torneo"""
    try:
        tournament = Tournament.query.get_or_404(tournament_id)
        ...
        teams = json.loads(request.form.get('teams'))
        ...
        # Salva i dati delle coppie e dei gironi
        day.set_config({
            'num_coppie': int(num_coppie),
            'num_gironi': int(num_gironi),
            'metodo_sorteggio': metodo_sorteggio,
        ...
        # Reindirizza alla pagina della giornata
        flash('Giornata creata con successo!', 'success')
        return redirect(url_for('tournaments.view_gironi_day', 
                              tournament_id=tournament_id,
                              day_id=day.id))
...
def gironi_seeded_pairing(tournament_id):
    """Pagina per la selezione delle teste di serie nel torneo a gironi"""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    if request.method == 'POST':
        ...
        return render_template('tournaments/gironi/pairing_animation.html',
                             ...
                             method="seeded",
                             method_title="Sorteggio con Teste di Serie",
                             method_description="Le squadre vengono formate assicurando che ogni testa di serie sia in un girone diverso",
                             tournament_id=tournament_id,
                             seeded_players=seeded_players,
                             num_coppie=num_coppie,
                             num_gironi=num_gironi,
                             form_action=url_for('tournaments.save_gironi_day_from_pairing', tournament_id=tournament_id))
